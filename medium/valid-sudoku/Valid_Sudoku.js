...
function isValidSudoku(board) {
    for (let i = 0; i < board.length; i++) {
        const validBoard = validateRow(board, i) && validateCol(board, i) && validateGrid(board, i);
        if (!validBoard)
            return false;
    }
    return true;
...
function validateRow(board, row) {
    ...
    for (let i = 0; i < board.length; i++) {
        ...
        if (validateCell(hash, digit) === false)
            return false;
    }
    return true;
}
...
function validateGrid(board, grid) {
    const hash = {};
...
function validateCell(hash, digit) {
    if (!digit)
        return;
    if (hash[digit])
        return false;
...
function gridStartingPoints(grid) {
